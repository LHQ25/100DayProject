        ...
        body: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: const Align(
                alignment: Alignment
                    .topRight, // 子组件在父组件中的起始位置。有两个常用的子类：Alignment和 FractionalOffset
                widthFactor:
                    1.5, // 用于确定Align 组件本身宽高的属性；分别乘以子元素的宽、高，最终的结果就是Align 组件的宽高。如果值为null，宽高将会占用尽可能多的空间。
                heightFactor: 1.5,
                child: FlutterLogo(
                  size: 60,
                ),
            ...
            const Text('Alignment -> Alignment(1, 0)'),
            Container(
              height: 120,
              width: 120,
              color: Colors.blue.shade50,
              child: const Align(
                alignment: Alignment(1,
                    0), // Alignment会以矩形的中心点作为坐标原点，即Alignment(0.0, 0.0) 。x、y的值从-1到1分别代表矩形左边到右边的距离和顶部到底边的距离，因此2个水平（或垂直）单位则等于矩形的宽（或高），如Alignment(-1.0, -1.0) 代表矩形的左侧顶点，而Alignment(1.0, 1.0)代表右侧底部终点，而Alignment(1.0, -1.0) 则正是右侧顶点，即Alignment.topRight。为了使用方便，矩形的原点、四个顶点，以及四条边的终点在Alignment类中都已经定义为了静态常量。
                child: FlutterLogo(
                  size: 60,
                ),
              ),
            ),
            const Text('FractionalOffset -> FractionalOffset(0.2, 0.6)'),
