...
func main3() {
	...
	*/
	...
	// 有缓冲的通道
	// 以在使用make函数初始化通道的时候为其指定通道的容量
	ch9 := make(chan int, 1) // 创建一个容量为1的有缓冲区通道
	ch9 <- 10
	fmt.Println("发送成功")
	// 只要通道的容量大于零，那么该通道就是有缓冲的通道，通道的容量表示通道中能存放元素的数量。
	// 就像你小区的快递柜只有那么个多格子，格子满了就装不下了，就阻塞了，等到别人取走一个快递员就能往里面放一个。
	// 可以使用内置的len函数获取通道内元素的数量，使用cap函数获取通道的容量，很少会这么做
	...
	ch_2 := make(chan int)
	go counter(ch_1)
	go squarer(ch_2, ch_1)
	printer(ch_2)
}

func recv(c chan int) {
	ret := <-c // 启用goroutine从通道接收值
